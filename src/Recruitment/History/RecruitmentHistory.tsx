...
import { timeToHuman } from "../Picker/Timer";

const RecruitmentHistory: FC<RecruitmentHistoryProps> = ({
    recHistory,
    characters,
    settings,
    setOutcome,
    ...
    const calculateOutcomes = (date: number, tags: string[], time = 9) => {
        const sc = getCharactersForFilters(characters, getFilters(tags, time));
        ...
        <>
                    ...
                    <>
                        ...
                        <div>
                            ...
                            {recHistory.reduce(
                                (p, n) =>
                                    p +
                                    (!filtersOut(n, historyFilters) ? 1 : 0),
                                0
                            )}
                        </div>
                        <div>
            ...
            <div
                className={`outcome-modal${
                    outcomeModal ? " outcome-show" : ""
                }`}
                onClick={(e) => {
                    const target = e.target as Element;
                    if (target.classList.contains("outcome-modal"))
                        setOutcomeModal(0);
                }}
            >
                <div className="outcome category">
                    <fieldset>
                        ...
                        <div className="crossed">
                            {(() => {
                                const hisRecData = recHistory.find(
                                    (r) => r?.date === outcomeModal
                                );
                                return hisRecData?.picked.map((r, n, a) => (
                                    <button
